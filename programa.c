...
typedef struct a{ /* Celula de uma lista de arestas */
    ...
    char   *nome_local; /* Nome do ponto de interesse (POI) */
    struct a *prox;
} Arest;
...
void imprimeGrafo(Vert G[], int ordem){
    ...
    printf("\nOrdem:   %d",ordem);
    printf("\nLista de Adjacencia (Vizinho | Distancia | Local):\n");

    for (i=0; i<ordem; i++){
        printf("\n    v%d ->", i);
        aux = G[i].prim;
        for( ; aux != NULL; aux = aux->prox) {
            printf(" v%d(%dm", aux->extremo2, aux->peso);
            if (aux->nome_local != NULL) {
                printf("/%s", aux->nome_local); 
            }
            printf(")");
        }
    }
...
void inicializarMapa(Vert *G, int ordemG) {
    ...
    adicionarLocalNaAresta(G, 4, 13, "We Vets Veterinario");
    adicionarLocalNaAresta(G, 10, 11, "1900 Pizzeria");
    adicionarLocalNaAresta(G, 15, 16, "Bacio di Latte");
    adicionarLocalNaAresta(G, 9, 18, "Petiskin do Bob");
    adicionarLocalNaAresta(G, 10, 19, "Academia CPN");
    adicionarLocalNaAresta(G, 13, 22, "Degas Pompeia");
    adicionarLocalNaAresta(G, 16, 25, "Cabelereiro Edmilson Araujo");
    adicionarLocalNaAresta(G, 20, 21, "Pizzaria Nogueira");
    adicionarLocalNaAresta(G, 25, 37, "Colegio Sagrado Coracao de Jesus");
    adicionarLocalNaAresta(G, 31, 32, "Santiago Padaria");
    adicionarLocalNaAresta(G, 32, 33, "Cazeco Bar");
    adicionarLocalNaAresta(G, 31, 41, "Galpao da Pizza");
    adicionarLocalNaAresta(G, 34, 44, "Hospital Sao Camilo");
    adicionarLocalNaAresta(G, 35, 45, "St. Marche Perdizes");
    adicionarLocalNaAresta(G, 36, 46, "Minuto Pao de Acucar");
    adicionarLocalNaAresta(G, 41, 42, "Bar do Gomes");
    adicionarLocalNaAresta(G, 47, 48, "Academia Smart Fit");
    adicionarLocalNaAresta(G, 22, 34, "Minha Casa");
}
...
int getDistanciaEntrePOIs(Vert G[], int ordem, Localidades poi_A, Localidades poi_B, int **melhor_caminho, int *tam_melhor_caminho) {
    ...
    *tam_melhor_caminho = 0;
    ...
    if (dist_vA1_vB1 != INT_MAX) {
        ...
        } else { if (caminho_atual) free(caminho_atual); }
    }
    caminho_atual = NULL;
...
void calcularPasseio(Vert G[], int ordem, Localidades minha_casa, Localidades locais[], int num_locais) {
    ...
    while (1) {
        ...
        if (modo_exibicao == 1 || modo_exibicao == 2) break; 
    ...
    if (modo_exibicao == 2) printf("INICIO (%s)", conjunto_visita[0].nome);
    ...
    for (i = 1; i < num_visita; i++) { 
        ...
        for (k = 0; k < num_visita; k++) {
            if (visitados[k] == 0) { 
                ...
                } else { if (caminho_teste) free(caminho_teste); }
            }
        }
        ...
        if (modo_exibicao == 1) {
            ...
            for(j = 0; j < tam_melhor_caminho_trecho; j++) {
                ...
                if (j < tam_melhor_caminho_trecho - 1) encontrarPOIsNoCaminho(melhor_caminho_trecho[j], melhor_caminho_trecho[j+1], locais, num_locais, loc_B.nome, 1);
            }
            printf(" -> %s\n", loc_B.nome);
        } else {
            if (tam_melhor_caminho_trecho > 0) printf(" -> v%d", melhor_caminho_trecho[0]);
    ...
    caminho_final = NULL;
    tam_caminho_final = 0;
    dist_volta = getDistanciaEntrePOIs(G, ordem, conjunto_visita[local_atual_idx], conjunto_visita[0], &caminho_final, &tam_caminho_final);
    ...
    if (modo_exibicao == 1) {
        ...
        for(j = 0; j < tam_caminho_final; j++) {
            ...
            if (j < tam_caminho_final - 1) encontrarPOIsNoCaminho(caminho_final[j], caminho_final[j+1], locais, num_locais, conjunto_visita[0].nome, 1);
        }
        printf(" -> %s\n", conjunto_visita[0].nome);
    } else {
        if (tam_caminho_final > 0) printf(" -> v%d", caminho_final[0]);
...
/* Funcao auxiliar para encontrar e imprimir POIs */
void encontrarPOIsNoCaminho(int v1, int v2, Localidades locais_todos[], int num_locais_todos, const char* nome_destino, int modo) {
    int i;
    for (i = 0; i < num_locais_todos; i++) {
        if ((locais_todos[i].vert1 == v1 && locais_todos[i].vert2 == v2) || (locais_todos[i].vert1 == v2 && locais_todos[i].vert2 == v1)) {
            if (strcmp(locais_todos[i].nome, nome_destino) != 0) {
                 if (modo == 1) printf(" -> (Passando por: %s)", locais_todos[i].nome);
                 else printf(" -> %s", locais_todos[i].nome);
            }
        }
    }
}
